...
# South
lat_337 <- par$Latitude[par$SiteID == "WCPCB-PAS337"][1]
# North
lat_56  <- par$Latitude[par$SiteID == "WCPCB-PAS056"][1]
par_subset <- par[
  par$Latitude < lat_337 &
    par$Latitude > lat_56, ]

# Select  locations, sampling date, lat, long and tPCB
tpcb <- data.frame(
  SiteName   = par_subset$SiteName,
  SampleDate = par_subset$SampleDate,
  Latitude   = par_subset$Latitude,
  Longitude  = par_subset$Longitude,
  tPCB       = par_subset$tPCB)
