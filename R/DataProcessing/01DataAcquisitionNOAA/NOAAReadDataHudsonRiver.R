...
# Hudson River coordinates (Albany, NY)
site_lat <- 42.6526
site_lon <- -73.7562
...
# Select the nearest major station (ALBANY INTERNATIONAL AIRPORT)
hur.station <- stations[stations$usaf == "725180", ]
...
years_needed <- sort(unique(as.integer(format(hur$SampleDate, "%Y"))))
years_needed

# NOAA station IDs
usaf <- as.character(hur.station$usaf[1])
wban <- as.character(hur.station$wban[1])
...
# Save
write.csv(hur, "Data/HudsonRiver/HudsonRiverMeteo.csv", row.names = FALSE)
