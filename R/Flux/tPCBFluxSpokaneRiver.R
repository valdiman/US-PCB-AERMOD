# Code to estimate total PCB fluxes from the Spokane River
...
SampleSite <- spr$SiteName
SampleDate <- spr$SampleDate
Latitude <- spr$Latitude
Longitude <- spr$Longitude

# Environmental conditions
tair <- spr$air_temp # [C]
twater <- spr$pred_water_temp_C # [C]
u <- spr$wind_speed # [m/s]
...
final.result <- function(MW.PCB, H0, C.PCB.water.vec, nOrtho.Cl, Kow,
  ...
  for (i in seq_along(C.PCB.water.vec)) {
    ...
    DOC <-  3 # [mg/L] average from similar rivers (Columbia, Sacramento, Colorado)
...
# Compute flux matrix -----------------------------------------------------
flux.mat <- matrix(NA, nrow = nrow(spr), ncol = ncol(C.PCB.water))
...
plot.flux <- ggplot(flux.df, aes(x = SampleDate, y = tPCB)) +
  ...
  annotate("text", x = as.Date("2016-08-01"), y = 400,
           label = "Spokane River", size = 5)
