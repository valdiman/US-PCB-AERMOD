...
fxr <- read.csv("Data/FoxRiver/FoxRiverMeteoWaterTempConcVF.csv")

# Calculate averages
C.PCB.water <- fxr[, 7:110]
SampleSite <- fxr$SiteName
SampleDate <- fxr$SampleDate
Latitude <- fxr$Latitude
Longitude <- fxr$Longitude

# Environmental conditions
tair <- fxr$air_temp # [C]
twater <- fxr$pred_water_temp_C # [C]
u <- fxr$wind_speed # [m/s]
# Modify u @6.7 m to @10 m
u10 <- (10.4/(log(6.7) + 8.1)) * u # [m/s]
P <- fxr$air_pressure # [atm]

Num.Congener <- ncol(C.PCB.water)
Num.Samples <- nrow(fxr) 
...
# Compute flux matrix -----------------------------------------------------
flux.mat <- matrix(NA, nrow = nrow(fxr), ncol = ncol(C.PCB.water))
...
for(i in 1:ncol(C.PCB.water)){
  flux.mat[, i] <- final.result(
    ...
    nOrtho.Cl[i], Kow[i],
    tair = fxr$air_temp,
    twater = fxr$pred_water_temp_C,
    u10 = (10.4/(log(6.7) + 8.1))*fxr$wind_speed,
    P = fxr$air_pressure
  )
}
...
plot.flux <- ggplot(flux.df, aes(x = SampleDate, y = tPCB)) +
  ...
  annotate("text", x = as.Date("2018-03-01"), y = 2500,
           label = "Fox River", size = 5)

# see plot
plot.flux

# Save plot in folder
ggsave("Output/Plot/FoxRiver/fluxtPCBFoxRiver2014_2018.png", plot = plot.flux, width = 16,
       height = 8, dpi = 500)
