...
SampleSite <- par$SiteName
SampleDate <- par$SampleDate
Latitude <- par$Latitude
Longitude <- par$Longitude

# Environmental conditions
tair <- par$air_temp # [C]
twater <- par$pred_water_temp_C # [C]
u <- par$wind_speed # [m/s]
# Modify u @6.7 m to @10 m
u10 <- (10.4/(log(6.7) + 8.1)) * u # [m/s]
P <- par$air_pressure # [atm]

Num.Congener <- ncol(C.PCB.water)
Num.Samples <- nrow(par) 
...
# Compute flux matrix -----------------------------------------------------
flux.mat <- matrix(NA, nrow = nrow(par), ncol = ncol(C.PCB.water))
...
for(i in 1:ncol(C.PCB.water)){
  flux.mat[, i] <- final.result(
    ...
    nOrtho.Cl[i], Kow[i],
    tair = par$air_temp,
    twater = par$pred_water_temp_C,
    u10 = (10.4/(log(6.7) + 8.1))*par$wind_speed,
    P = par$air_pressure
  )
}
...
plot.flux <- ggplot(flux.df, aes(x = SampleDate, y = tPCB)) +
  ...
  annotate("text", x = as.Date("2019-04-01"), y = 10000,
           label = "Passaic River", size = 5)
...
# Save data ---------------------------------------------------------------
write.csv(flux.df, "Output/Data/PassaicRiver/tFluxPassaicRiver_2018_2019.csv",
          row.names = FALSE)
