...
anr.station <- stations[stations$usaf == "724050", ]
...
years_needed <- sort(unique(as.integer(format(anr$SampleDate, "%Y"))))
years_needed

# NOAA station IDs
usaf <- as.character(anr.station$usaf[1])
wban <- as.character(anr.station$wban[1])
