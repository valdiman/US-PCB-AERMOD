...
# Select the nearest major station (NEWARK LIBERTY INTERNATIONAL AP)
par.station <- stations[stations$usaf == "725020", ]

# Read Hudson River Data
par <- read.csv("Data/PassaicRiver/PassaicRiver.csv")
par$SampleDate <- as.Date(par$SampleDate)

# Get unique years needed
years_needed <- sort(unique(as.integer(format(par$SampleDate, "%Y"))))
years_needed

# NOAA station IDs
usaf <- as.character(par.station$usaf[1])
wban <- as.character(par.station$wban[1])
