...
# Housatonic River coordinates (Pittsfield, MA)
site_lat <- 42.4501
site_lon <- -73.2454
...
# Select the nearest major station (PITTSFIELD MUNICIPAL AP)
hor.station <- stations[stations$usaf == "744104", ]

# Read Housatonic River Data
hor <- read.csv("Data/HousatonicRiver/HousatonicRiver.csv")
hor$SampleDate <- as.Date(hor$SampleDate)

# Get unique years needed
years_needed <- sort(unique(as.integer(format(hor$SampleDate, "%Y"))))
years_needed
# Select only years >= 2006. No data available before 2006
years_needed <- years_needed[years_needed >= 2006]

# NOAA station IDs
usaf <- as.character(hor.station$usaf[1])
wban <- as.character(hor.station$wban[1])
