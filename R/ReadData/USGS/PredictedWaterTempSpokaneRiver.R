...
# ---- 1. Read data ----
spr <- read.csv("Data/Spokane/SpokaneRiver.csv", stringsAsFactors = FALSE)
spr$SampleDate <- as.Date(spr$SampleDate)   # ensure Date

# Keep only 2000+
spr <- spr %>% filter(SampleDate >= as.Date("2000-01-01"))

# Coordinates (Spokane, WA)
lon <- -117.4260
lat <- 47.6588
...
yr1 <- year(min(spr$SampleDate))
yr2 <- year(max(spr$SampleDate))
...
out_pred <- out %>%
  select(all_of(names(spr)), pred_temp_C)

# ---- 5. Save result ----
write.csv(out_pred, "Data/Spokane/SpokaneRiver_temp.csv", row.names = FALSE)
