...
# ---- 1. Read data ----
par <- read.csv("Data/PassaicRiver/PassaicRiver.csv", stringsAsFactors = FALSE)
par$SampleDate <- as.Date(par$SampleDate)   # ensure Date

# Keep only 2000+
par <- par %>% filter(SampleDate >= as.Date("2000-01-01"))
...
yr1 <- year(min(par$SampleDate))
yr2 <- year(max(par$SampleDate))
...
out_pred <- out %>%
  select(all_of(names(par)), pred_temp_C)

# ---- 5. Save result ----
write.csv(out_pred, "Data/PassaicRiver/PassaicRiver_temp.csv", row.names = FALSE)
