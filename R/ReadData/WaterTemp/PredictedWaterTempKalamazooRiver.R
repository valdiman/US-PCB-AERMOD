...
# ---- 1. Read data ----
kar <- read.csv("Data/Kalamazoo/KalamazooRiver.csv", stringsAsFactors = FALSE)
kar$SampleDate <- as.Date(kar$SampleDate)   # ensure Date

# Keep only 2000+
kar <- kar %>% filter(SampleDate >= as.Date("2000-01-01"))

# Coordinates (Kalamazoo, MI)
lon <- -85.5872
lat <- 42.2917
...
yr1 <- year(min(kar$SampleDate))
yr2 <- year(max(kar$SampleDate))
...
out_pred <- out %>%
  select(all_of(names(kar)), pred_temp_C)

# ---- 5. Save result ----
write.csv(out_pred, "Data/Kalamazoo/KalamazooRiver_temp.csv", row.names = FALSE)
