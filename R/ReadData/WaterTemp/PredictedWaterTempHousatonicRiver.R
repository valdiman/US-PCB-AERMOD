...
# ---- 1. Read data ----
hor <- read.csv("Data/HousatonicRiver/HousatonicRiver.csv", stringsAsFactors = FALSE)
hor$SampleDate <- as.Date(hor$SampleDate)   # ensure Date

# Keep only 2000+
hor <- hor %>% filter(SampleDate >= as.Date("2000-01-01"))

# Coordinates (Pittsfield)
lat <- 42.4501
lon <- -73.2454
...
yr1 <- year(min(hor$SampleDate))
yr2 <- year(max(hor$SampleDate))
...
out_pred <- out %>%
  select(all_of(names(hor)), pred_temp_C)

# ---- 5. Save result ----
write.csv(out_pred, "Data/HousatonicRiver/HousatonicRiver_temp.csv", row.names = FALSE)
